...
def parse_args():
    parser = argparse.ArgumentParser(
        description=('Import CTFd challenges and their attachments '
                     'to a DB from a YAML formated specification '
                     'file and an associated attachment directory'))
    parser.add_argument(
        '--app-root', dest='app_root', type=str,
        help=('app_root directory for the CTFd Flask app '
              '(default: 2 directories up from this script)'),
        default=None)
    parser.add_argument(
        '-d', dest='db_uri', type=str,
        help="URI of the database where the challenges should be stored")
    parser.add_argument(
        '-F', dest='dst_attachments', type=str,
        help="directory where challenge attachment files should be stored")
    parser.add_argument(
        '-i', dest='in_file', type=str,
        help="name of the input YAML file (default: export.yaml)",
        default="export.yaml")
    parser.add_argument(
        '--skip-on-error', dest="exit_on_error", action='store_false',
        help=('If set, the importer will skip the importing challenges '
              'which have errors rather than halt.'),
        default=True)
    parser.add_argument(
        '--move', dest="move", action='store_true',
        help=('if set the import proccess will move files rather than '
              'copy them'),
        default=False)
...
def process_args(args):
    ...
    if not args.dst_attachments:
        args.dst_attachments = os.path.join(app.root_path,
...
def import_challenges(
        in_file, dst_attachments, exit_on_error=True, move=False):
            ...
            for req_field in REQ_FIELDS:
                if req_field not in chal:
                    if exit_on_error:
                        ...
                        print("Skipping challenge: Missing field '{}'"
                              .format(req_field))
                        skip = True
                        break
            if skip:
