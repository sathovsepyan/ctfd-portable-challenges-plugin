...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export a DB full of CTFd challenges and theirs attachments into a portable YAML formated specification file and an associated attachment directory"
    )
    parser.add_argument(
        "--app-root",
        dest="app_root",
    ...
    parser.add_argument(
        ...
        dest="db_uri",
        type=str,
        help="URI of the database where the challenges are stored",
    )
    parser.add_argument(
        "-F",
        dest="src_attachments",
        type=str,
        help="directory where challenge attachment files are stored",
    )
    parser.add_argument(
        "-o",
        dest="out_file",
        type=str,
        help="name of the output YAML file (default: challenges.yaml)",
    ...
    parser.add_argument(
        ...
        type=str,
        help="directory for output challenge attachments (default: [OUT_FILENAME].d)",
        default=None,
    )
    parser.add_argument(
        "--tar", dest="tar", help="if present, output to tar file", action="store_true"
    )
    parser.add_argument(
        "--gz",
        dest="gz",
        help="if present, compress the tar file (only used if '--tar' is on)",
        action="store_true",
    )
...
def process_args(args):
    if not (args.db_uri and args.src_attachments):
        ...
        )  # Enable imports of CTFd modules
    ...
    for src_path, dst_path in file_map.items():
        ...
        if not os.path.isdir(dst_dir):
            if os.path.exists(dst_dir):
                raise RuntimeError(
                    "Output directory name exists, but is not a directory: %s" % dst_dir
                )
            os.makedirs(dst_dir)
...
if __name__ == "__main__":
    ...
    with app.app_context():
        ...
        out_stream.write(
            export_challenges(
                args.out_file, args.dst_attachments, args.src_attachments, tarfile
            )
        )
